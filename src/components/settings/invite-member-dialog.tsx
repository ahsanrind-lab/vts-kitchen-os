...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our VTS Kitchen OS account',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    // for users in multi-team contexts where "our VTS Kitchen OS account"
    // wouldn't be enough to disambiguate.
    const accountName = result?.accountName ?? 'our VTS Kitchen OS account';
    const message = `Join ${accountName} on VTS Kitchen OS using this link (valid for ${result?.expiresInDays} days): ${url}`;
          ...
          <>
            ...
            <div className="space-y-3 py-2">
              ...
              {/* Anchor styled with `buttonVariants` rather than wrapping
                  in <Button asChild>. The upstream Button is the Base UI
